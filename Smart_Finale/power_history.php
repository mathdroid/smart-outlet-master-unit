<?php    
    ...
    $query = mysql_query("SELECT * FROM powerlog_history")or die(mysql_error()); 
    ...
    function UpdateTotal($day,$kwh,$budget) {       
        $retval = mysql_query("UPDATE total SET kwh ='".$kwh."', budget ='".$budget."' WHERE days = '".$day."' AND sid=1");
        if(! $retval ){
...
//database handle
	$sth = mysql_query("SELECT budget FROM powerlog_history WHERE sid=1 ORDER BY timestamp");
    if($sth === FALSE) {
    ...
    $sth = mysql_query("SELECT kwh FROM powerlog_history WHERE sid=1 ORDER BY timestamp");
    $rows1 = array();
    $rows1['name'] = 'kwh';
