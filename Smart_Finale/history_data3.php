...
//	//database handle
//	$sth = mysql_query("SELECT budget FROM total WHERE sid=3 ORDER BY days");
//    if($sth === FALSE) {
...
//    }
//    
//    $sth = mysql_query("SELECT kwh FROM total WHERE sid=3 ORDER BY days");
//    $rows1 = array();
//    $rows1['name'] = 'kwh';
...
//	$dbpass		= "smartsocket";
    ...
    function UpdateTotal($day,$kwh,$budget) {       
        $retval = mysql_query("UPDATE total SET kwh ='".$kwh."', budget ='".$budget."' WHERE days = '".$day."' AND sid=3");
        if(! $retval ){
