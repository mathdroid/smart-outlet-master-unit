<?php    
    ...
    $query = mysql_query("SELECT * FROM powerlog_history WHERE sid=1")or die(mysql_error()); 
...
//database handle
	$sth = mysql_query("SELECT budget FROM total WHERE sid=1 ORDER BY days");
    if($sth === FALSE) {
    ...
    $sth = mysql_query("SELECT kwh FROM total WHERE sid=1 ORDER BY days");
    $rows1 = array();
    $rows1['name'] = 'kwh';
